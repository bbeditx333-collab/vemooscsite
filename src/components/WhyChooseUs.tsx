...
export default function WhyChooseUs() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      // Headline words rise from behind their own baseline.
      const headline = rootRef.current?.querySelector<HTMLElement>(".why-headline");
      if (headline) revealHeadline(headline, { start: "top 82%" });

      gsap.set(".why-reveal", { opacity: 0, y: 32 });
      // "Us" holds the SVG swash, so it animates as a unit rather than
      // being word-split (which would wipe the inline SVG).
      gsap.set(".why-accent", { opacity: 0, y: 40 });
      gsap.set(".why-card", { opacity: 0, y: 64, rotateX: 10 });
      ...
      gsap
        ...
        .to(".why-accent", { opacity: 1, y: 0, duration: 0.9 }, 0.12)
        .to(".why-swash", { opacity: 1, scale: 1, rotate: 0, duration: 1 }, 0.3)
        // Cards tip up into place rather than simply fading — reads as
        // physical panels settling, not another fade-in.
        .to(
          ".why-card",
          { opacity: 1, y: 0, rotateX: 0, duration: 1, stagger: 0.18 },
          0.25
        );

      // Feature bullets tick in one by one once their card has landed.
      gsap.utils.toArray<HTMLElement>(".why-card").forEach((card) => {
        gsap.fromTo(
          card.querySelectorAll(".why-point"),
          { opacity: 0, x: -14 },
          {
            opacity: 1,
            x: 0,
      ...
      // Pointer-follow tilt: the card leans toward the cursor, then settles.
    ...
    >
      <div className="relative z-10 mx-auto max-w-6xl px-6 md:px-14">
        <div className="text-center">
          ...
          <h2 className="relative inline-block text-4xl font-bold tracking-tight sm:text-5xl md:text-6xl">
            <span className="why-headline inline-block">Why Choose</span>{" "}
            <span className="why-accent relative inline-block text-brand">
              Us
              <svg
            ...
            <div className="p-8 md:p-10">
              ...
              <ul className="mt-6 space-y-3">
                {DELIVERY_POINTS.map((point) => (
                  <li key={point} className="why-point flex items-start gap-3 text-sm text-foreground-muted md:text-base">
